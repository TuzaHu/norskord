...
class PREPPLingoGame:
    def __init__(self):
        ...
        # Load words dynamically (after chapter system is initialized)
    ...
    def initialize_colors(self):
        """Initialize the futuristic color scheme"""
        self.colors = {
            # Primary colors - Cyber blue theme
            'primary': '#00d4ff',      # Electric blue
            'primary_dark': '#0099cc',  # Darker blue
            'primary_light': '#66e6ff', # Light blue
            'accent': '#ff6b35',       # Orange accent
            'accent_dark': '#e55a2b',  # Dark orange
            
            # Secondary colors
            'secondary': '#6366f1',    # Purple
            'secondary_light': '#a5b4fc', # Light purple
            ...
            'dark_light': '#1e293b',   # Lighter dark
            'gray': '#64748b',         # Gray
    ...
    def load_words_data(self):
        """Load words dynamically from chapter-based system"""
        words_data = {}
        ...
        if self.chapter_manager:
            try:
                ...
                for chapter in chapters:
                    if chapter['unlocked']:
                        chapter_words = self.chapter_manager.get_chapter_words(chapter['folder'])
        ...
        try:
            ...
            print(f"⚠️ Fallback: Loaded {len(words_data)} words from merged_log.txt")
        except FileNotFoundError:
            print(f"❌ Error: Neither chapter system nor {self.merged_log_file} found")
            return {}
    ...
    def update_stats(self, difficulty, correct):
        ...
        if correct:
            ...
            self.game_stats["total_xp"] += 10  # Award 10 XP per correct answer
    ...
    def setup_gui(self):
        """Setup the game GUI with futuristic PREPP-Lingo design"""
        # Clear existing widgets without destroying root
        for widget in self.root.winfo_children():
            widget.destroy()
        
        # Colors are now initialized in __init__ method
        
        # Configure modern styles
        style = ttk.Style()
        style.theme_use('clam')
        
        # Modern button styles with glass morphism effect
        style.configure('PREPP.TButton', 
        ...
        style.configure('Secondary.TButton', 
                       ...
                       background=self.colors['dark_light'],
                       foreground=self.colors['white'],
                       borderwidth=1,
        ...
        # Skip gradient background for now to avoid canvas issues
        # self.create_gradient_background()
        
        # Header with futuristic design
        header_frame = tk.Frame(self.main_container, bg=self.colors['dark'])
        header_frame.pack(fill=tk.X, pady=(30, 20), padx=30)
        
        # Top row with stats and settings
        top_row = tk.Frame(header_frame, bg=self.colors['dark'])
        top_row.pack(fill=tk.X, pady=(0, 20))
        
        # Streak display with modern styling
        streak_frame = tk.Frame(top_row, bg=self.colors['dark_light'], relief='flat', bd=0)
        streak_frame.pack(side=tk.LEFT, padx=(0, 10))
        
        streak_label = tk.Label(streak_frame, 
                               text=f"⚡ STREAK: {self.game_stats['current_streak']} DAYS", 
                               font=('SF Pro Display', 11, 'bold'), 
                               fg=self.colors['accent'], 
                               bg=self.colors['dark_light'],
                               padx=15, pady=8)
        streak_label.pack()
        
        # XP display
        xp_frame = tk.Frame(top_row, bg=self.colors['dark_light'], relief='flat', bd=0)
        xp_frame.pack(side=tk.LEFT, padx=(0, 10))
        
        xp_label = tk.Label(xp_frame, 
                           text=f"💎 XP: {self.game_stats.get('total_xp', 0)}", 
                           font=('SF Pro Display', 11, 'bold'), 
        ...
        # Settings button (top right) with modern design
        settings_button = tk.Button(top_row, text="⚙", 
        ...
        subtitle_label = tk.Label(logo_frame, text="ADVANCED NORWEGIAN DICTATION TRAINING", 
                                 font=('SF Pro Display', 12), 
                                 fg=self.colors['accent'], 
                                 bg=self.colors['dark'])
        subtitle_label.pack()
        ...
        # Status indicator
        status_frame = tk.Frame(logo_frame, bg=self.colors['dark_light'], relief='flat', bd=0)
    ...
    def create_gradient_background(self):
        ...
        # Send canvas to back
        self.main_container.lower(self.bg_canvas)
        
        # Create gradient effect with multiple rectangles
        width = self.root.winfo_width() if self.root.winfo_width() > 1 else 800
        height = self.root.winfo_height() if self.root.winfo_height() > 1 else 600
        
        for i in range(height):
            ratio = i / height
            r = int(15 + ratio * 10)  # 15 to 25
            g = int(23 + ratio * 15)  # 23 to 38
            b = int(42 + ratio * 20)  # 42 to 62
            color = f"#{r:02x}{g:02x}{b:02x}"
            self.bg_canvas.create_line(0, i, width, i, fill=color, width=1)
    
    def create_futuristic_logo(self):
        """Create a futuristic PREPP-Lingo logo"""
        canvas = self.logo_canvas
        
        # Clear canvas
        canvas.delete("all")
        
        # Create hexagonal logo background
        ...
        for i in range(6):
            ...
            x = center_x + size * 0.8 * math.cos(angle)
            y = center_y + size * 0.8 * math.sin(angle)
            points.extend([x, y])
        
        canvas.create_polygon(points, fill=self.colors['primary'], outline=self.colors['primary_light'], width=2)
        ...
        canvas.create_polygon(inner_points, fill=self.colors['dark'], outline=self.colors['primary_light'], width=1)
        
        # Center dot
        canvas.create_oval(center_x-3, center_y-3, center_x+3, center_y+3, 
                          fill=self.colors['accent'], outline='')
        
        # Add glow effect lines
        for i in range(6):
            angle = i * 60 * 3.14159 / 180
            start_x = center_x + size * 0.5 * math.cos(angle)
            start_y = center_y + size * 0.5 * math.sin(angle)
            end_x = center_x + size * 1.2 * math.cos(angle)
            end_y = center_y + size * 1.2 * math.sin(angle)
            canvas.create_line(start_x, start_y, end_x, end_y, 
                             fill=self.colors['primary_light'], width=1)
    
    def setup_progress_section(self):
        """Setup the progress section with modern design"""
        # Progress section with glass morphism effect
        progress_frame = tk.Frame(self.main_container, bg=self.colors['dark_light'], 
                                 relief='flat', bd=0)
        progress_frame.pack(fill=tk.X, pady=(0, 20), padx=30)
        
        # Hearts (lives) display
        hearts_frame = tk.Frame(progress_frame, bg=self.colors['dark_light'])
        hearts_frame.pack(side=tk.LEFT, padx=20, pady=15)
        
        hearts_label = tk.Label(hearts_frame, text="HEARTS:", 
                               font=('SF Pro Display', 11, 'bold'), 
                               fg=self.colors['gray'], 
                               bg=self.colors['dark_light'])
        hearts_label.pack(side=tk.LEFT)
        ...
        # Circular progress indicator for action mode
        progress_canvas_frame = tk.Frame(progress_frame, bg=self.colors['dark_light'])
        progress_canvas_frame.pack(side=tk.RIGHT, padx=20, pady=15)
        
        self.progress_canvas = tk.Canvas(progress_canvas_frame, width=50, height=50, 
                                        bg=self.colors['dark_light'], highlightthickness=0)
        ...
        self.settings_info_label = tk.Label(progress_frame, 
                                           text="● PRACTICE MODE • EASY • 10 WORDS • TRANSLATIONS ON", 
                                           font=('SF Pro Display', 10, 'bold'), 
                                           fg=self.colors['accent'], 
                                           bg=self.colors['dark_light'],
                                           padx=20, pady=15)
    ...
    def setup_lesson_card(self):
        """Setup the main lesson card with modern design"""
        # Main lesson card with glass morphism
        lesson_card = tk.Frame(self.main_container, bg=self.colors['dark_light'], 
                              relief='flat', bd=0)
        lesson_card.pack(fill=tk.BOTH, expand=True, pady=(0, 20), padx=30)
        
        # Lesson content area
        lesson_content = tk.Frame(lesson_card, bg=self.colors['dark_light'])
        lesson_content.pack(fill=tk.BOTH, expand=True, padx=40, pady=40)
        
        # Word display with modern styling
        self.word_display = tk.Label(lesson_content, 
                                    text="🎯 Ready to start?", 
                                    font=('SF Pro Display', 28, 'bold'), 
                                    fg=self.colors['white'], 
                                    bg=self.colors['dark_light'],
                                    wraplength=600)
        self.word_display.pack(pady=(0, 30))
        
        # Translation display
        self.translation_display = tk.Label(lesson_content, 
                                           text="", 
                                           font=('SF Pro Display', 16), 
                                           fg=self.colors['primary'], 
                                           bg=self.colors['dark_light'])
        self.translation_display.pack(pady=(0, 30))
        
        # Input field with modern styling
        input_frame = tk.Frame(lesson_content, bg=self.colors['dark_light'])
        ...
        self.input_entry = tk.Entry(input_frame, 
                                   ...
                                   relief='flat', bd=0,
                                   width=30)
        self.input_entry.pack(pady=15, padx=20)
        
        # Bind Enter key
        self.input_entry.bind('<Return>', lambda event: self.submit_answer())
    
    def setup_controls(self):
        """Setup control buttons with modern design"""
        # Control buttons frame
        controls_frame = tk.Frame(self.main_container, bg=self.colors['dark'])
        controls_frame.pack(fill=tk.X, pady=(0, 30), padx=30)
        
        # Button container
        button_container = tk.Frame(controls_frame, bg=self.colors['dark'])
        ...
        # Submit button
        self.submit_button = ttk.Button(button_container, 
                                       text="✓ SUBMIT", 
        ...
        # Hint button
        self.hint_button = ttk.Button(button_container, 
                                     text="💡 HINT", 
        ...
        self.session_button = ttk.Button(button_container, 
                                        text="🚀 START SESSION", 
                                        style='PREPP.TButton',
                                        command=self.start_game)
        self.session_button.pack(side=tk.LEFT, padx=(0, 15))
        
        # Progress display
        progress_frame = tk.Frame(controls_frame, bg=self.colors['dark'])
    ...
    def update_status_with_animation(self, message, color=None):
        """Update status with animated color transition"""
        if color is None:
        ...
        self.root.after(500, lambda: self.status_label.config(fg=original_color))
        
        # Update text
        self.status_label.config(text=f"● {message}")
    
    def add_glow_effect(self, widget, color):
        """Add a subtle glow effect to widgets"""
        # This would require more complex canvas operations
        # For now, we'll use a simple border effect
        widget.config(highlightbackground=color, highlightthickness=2)
        self.root.after(200, lambda: widget.config(highlightthickness=0))
    
    def open_settings(self):
        """Open the futuristic settings window"""
        # Create settings window with dark theme
        settings_window = tk.Toplevel(self.root)
        settings_window.title("PREPP-Lingo Settings")
        ...
        y = (settings_window.winfo_screenheight() // 2) - (900 // 2)
        settings_window.geometry(f"700x900+{x}+{y}")
        
        # Create scrollable frame with dark theme
        canvas = tk.Canvas(settings_window, bg=self.colors['dark'], highlightthickness=0)
        ...
        header_frame = tk.Frame(scrollable_frame, bg=self.colors['dark_light'], relief='flat', bd=0)
        header_frame.pack(fill=tk.X, padx=20, pady=(20, 30))
        
        title_label = tk.Label(header_frame, 
                              text="⚙ PREPP-LINGO SETTINGS", 
        ...
        # Title
        title_label = tk.Label(main_frame, text="⚙️ PREPP-Lingo Settings", 
                              font=('SF Pro Display', 18, 'bold'), 
                              fg=self.colors['primary'], bg=self.colors['dark_light'])
        title_label.pack(pady=(0, 20))
        
        # Chapter selection section (more compact)
        chapter_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        chapter_frame.pack(fill=tk.X, pady=(0, 15))
        
        tk.Label(chapter_frame, text="Velg Kapittel:", 
                font=('SF Pro Display', 12, 'bold'), 
                fg=self.colors['gray'], bg=self.colors['dark_light']).pack(anchor='w', pady=(0, 5))
        
        # Chapter dropdown frame
        chapter_dropdown_frame = tk.Frame(chapter_frame, bg=self.colors['dark_light'])
        ...
        # Difficulty section
        difficulty_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        difficulty_frame.pack(fill=tk.X, pady=(0, 20))
        
        tk.Label(difficulty_frame, text="Velg Vanskelighetsgrad:", 
                font=('Arial', 14, 'bold'), 
                fg=self.colors['gray'], bg=self.colors['dark_light']).pack(anchor='w', pady=(0, 10))
        ...
        difficulty_buttons_frame = tk.Frame(difficulty_frame, bg=self.colors['dark_light'])
        ...
        def update_button_styles():
            ...
            for btn in [easy_radio, medium_radio, hard_radio]:
                btn.config(bg=self.colors['dark_light'], fg=self.colors['gray'], relief='flat')
            
            # Highlight the selected button
            if current == "easy":
                easy_radio.config(bg=self.colors['primary_light'], fg='white', relief='raised')
            elif current == "medium":
                medium_radio.config(bg=self.colors['accent'], fg='white', relief='raised')
            elif current == "hard":
                hard_radio.config(bg=self.colors['error'], fg='white', relief='raised')
        ...
        words_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        ...
        translation_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        ...
        checkbox_frame = tk.Frame(translation_frame, bg=self.colors['dark_light'])
        ...
        help_label = tk.Label(translation_frame, 
                             ...
                             font=('SF Pro Display', 12), 
                             fg=self.colors['gray'], bg=self.colors['dark_light'])
        help_label.pack(anchor='w', pady=(10, 0))
        
        # Game mode section
        mode_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        ...
        mode_buttons_frame = tk.Frame(mode_frame, bg=self.colors['dark_light'])
        ...
        def update_mode_styles():
            ...
            # Highlight the selected button
            if current == "practice":
                practice_radio.config(bg=self.colors['primary_light'], fg='white', relief='raised')
            elif current == "action":
                action_radio.config(bg=self.colors['error'], fg='white', relief='raised')
        ...
        # Word counts section
        counts_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        counts_frame.pack(fill=tk.X, pady=(0, 20))
        
        tk.Label(counts_frame, text="Tilgjengelige Ord:", 
                font=('Arial', 14, 'bold'), 
                fg=self.colors['gray'], bg=self.colors['dark_light']).pack(anchor='w', pady=(0, 8))
        ...
        # Buttons
        button_frame = tk.Frame(main_frame, bg=self.colors['dark_light'])
        ...
        cancel_button = tk.Button(button_frame, text="❌ Avbryt", 
                                 font=('SF Pro Display', 12, 'bold'),
                                 bg=self.colors['light_gray'], fg=self.colors['gray'],
                                 relief='flat', bd=0,
                                 padx=20, pady=8,
    ...
    def use_hint(self):
        ...
        if hasattr(self, 'result_text') and self.result_text.winfo_exists():
            ...
            self.result_text.tag_config("hint", foreground=self.colors['accent'])
    ...
    def play_audio(self):
        ...
        def audio_worker():
            audio_path = self.get_audio_file_path(self.current_audio_file)
    ...
    def get_audio_file_path(self, audio_file):
        """Get the full path to the audio file, checking chapter folders first"""
        # Check chapter folders first
        if self.chapter_manager:
            try:
                chapters = self.chapter_manager.get_available_chapters()
                for chapter in chapters:
                    if chapter['unlocked']:
                        chapter_audio_path = os.path.join("chapters", chapter['folder'], "audio", audio_file)
                        if os.path.exists(chapter_audio_path):
                            return chapter_audio_path
            except Exception as e:
                print(f"Error checking chapter audio: {e}")
        
        # Fallback to main audio directory
        main_audio_path = os.path.join(self.audio_directory, audio_file)
    ...
    def handle_timeout(self):
        ...
        if hasattr(self, 'result_text') and self.result_text.winfo_exists():
            ...
            self.result_text.tag_config("timeout", foreground=self.colors['error'])
            self.result_text.config(state='disabled')
    ...
    def handle_action_timeout(self):
        ...
        if hasattr(self, 'result_text') and self.result_text.winfo_exists():
            ...
            self.result_text.tag_config("game_over", foreground=self.colors['error'])
            self.result_text.config(state='disabled')
    ...
    def submit_answer(self, event=None):
        ...
        if player_answer.lower() == correct_answer.lower():
            ...
            if hasattr(self, 'result_text') and self.result_text.winfo_exists():
                ...
                self.result_text.tag_config("correct", foreground=self.colors['primary'])
                self.result_text.config(state='disabled')
        ...
        def reveal_worker():
            ...
            for i, letter in enumerate(correct_answer):
                ...
                if hasattr(self, 'result_text') and self.result_text.winfo_exists():
                    ...
                    self.root.after(0, lambda: self.result_text.tag_config("incorrect", foreground=self.colors['error']))
    ...
    def show_results_screen(self, total_words, correct_words, incorrect_words, accuracy):
        ...
        results_frame = tk.Frame(self.content_frame, bg=self.colors['dark_light'])
        ...
        stats_frame = tk.Frame(results_frame, bg=self.colors['dark_light'], relief='solid', bd=2)
        stats_frame.pack(fill=tk.X, pady=(0, 15), padx=10)
        
        tk.Label(stats_frame, text="📊 Samlet Resultat", 
                font=('Arial', 14, 'bold'), 
                fg=self.colors['gray'], bg=self.colors['dark_light']).pack(pady=8)
...
• Streak: {self.game_stats['current_streak']} dager"""
        ...
        if accuracy > 90:
            ...
            performance_color = self.colors['primary']
        elif accuracy > 70:
        ...
        if self.incorrect_answers:
            incorrect_frame = tk.Frame(answers_frame, bg=self.colors['dark_light'], relief='solid', bd=2)
            ...
            tk.Label(incorrect_frame, text="❌ Ord som Trenger Øvelse", 
                    font=('SF Pro Display', 12, 'bold'), 
                    fg=self.colors['error'], bg=self.colors['dark_light']).pack(pady=6)
        ...
        # Action buttons
        button_frame = tk.Frame(results_frame, bg=self.colors['dark_light'])
